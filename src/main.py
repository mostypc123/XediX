...
class TextEditor(wx.Frame):
    def __init__(self, *args, **kwargs):
        ...
        if wx.Platform == "__WXMSW__":
            ...
            self.active_color = config.get(
                "headerActive", "#EDF0F2"
            )  # Default values if not found
            self.inactive_color = config.get(
                "headerInactive", "#b3d0e4"
            )  # Default values if not found
    ...
    def InitUI(self):
        ...
        self.file_list = wx.ListBox(self.sidebar, style=wx.LB_SINGLE | wx.BORDER_THEME)
        ...
        self.default_message = wx.StaticText(
            self.main_panel, label="Open a File first", style=wx.ALIGN_CENTER
        )
        ...
        sidebar_vbox.Add(
            new_file_btn, proportion=0, flag=wx.EXPAND | wx.RIGHT | wx.BOTTOM, border=10
        )
        sidebar_vbox.Add(
            self.file_list, proportion=1, flag=wx.EXPAND | wx.RIGHT, border=10
        )
    ...
    def CreateMenuBar(self):
        ...
        folder_item = fileMenu.Append(
            wx.ID_ANY, "&Open Folder\tCtrl+Shift+O", "Open Folder"
        )
        fileMenu.AppendSeparator()
        pylint_item = fileMenu.Append(
            wx.ID_ANY, "&Run PyLint\tCtrl+P", "Run pylint on code"
        )
        ...
        paste_item = editMenu.Append(
            wx.ID_PASTE, "&Paste\tCtrl+V", "Paste from clipboard"
        )
        editMenu.AppendSeparator()

        find_replace_item = editMenu.Append(
            wx.ID_FIND, "&Find and Replace\tCtrl+F", "Find and replace text"
        )
        jump_line_item = editMenu.Append(
            wx.ID_ANY, "&Jump to Line\tCtrl+G", "Jump to a specific line number"
        )
        ...
        # Merge Resolver
        git_submenu.AppendSeparator()
        merge_resolve_item = git_submenu.Append(
            wx.ID_ANY, "Merge Resolver", "Resolve Merge Conflicts"
        )
        ...
        configMenu = wx.Menu()
        customize_item = configMenu.Append(
            wx.ID_ANY, "&Customize manually\tCtrl+Shift+C", "Customize the UI"
        )
        settings_item = configMenu.Append(wx.ID_ANY, "&Settings", "Open Settings")

        projectMenu = wx.Menu()
        init_project_item = projectMenu.Append(
            wx.ID_ANY, "&Init Project", "Initialize a new project"
        )
        init_python_item = projectMenu.Append(
            wx.ID_ANY, "&Init Python", "Initialize a new Python project"
        )
        init_git_item = projectMenu.Append(
            wx.ID_ANY, "&Init Git", "Initialize a new Git project"
        )
    ...
    def OnOpenFolder(self, event, event_or_path):
        ...
        if isinstance(event_or_path, str):
            ...
            dlg = wx.DirDialog(
                self,
                "Choose a directory, if does not exist, it will be created",
        ...
        try:
            ...
            self.SetStatusText(
                f"    Error: No permission to create/access directory: {path}"
            )
            return
        except OSError as e:
    ...
    def OnFileListRightClick(self, event):
        ...
        if index != wx.NOT_FOUND:
            ...
            self.Bind(
                wx.EVT_MENU, lambda evt: self.ShowFileProperties(index), props_item
            )
    ...
    def ShowFileProperties(self, index):
        ...
        vbox.Add(
            wx.StaticText(panel, label=f"Size: {size} bytes"),
            flag=wx.LEFT | wx.RIGHT,
            border=20,
        )
        vbox.Add(
            wx.StaticText(panel, label=f"Created: {ctime}"),
            flag=wx.LEFT | wx.RIGHT,
            border=20,
        )
        vbox.Add(
            wx.StaticText(panel, label=f"Modified: {mtime}"),
        ...
        if selected_index != wx.NOT_FOUND:
            ...
            if dialog.ShowModal() == wx.ID_OK:
                ...
                try:
                    ...
                    wx.MessageBox(
                        f"Error renaming file: {str(e)}", "Error", wx.OK | wx.ICON_ERROR
                    )
            dialog.Destroy()
    ...
    def OnDeleteFile(self, event):
        ...
        if selected_index != wx.NOT_FOUND:
            ...
            # Bold label for instruction
            label = wx.StaticText(
                panel, label=f"Are you sure you want to delete '{filename}'?"
            )
            font = label.GetFont()
            font.SetWeight(wx.FONTWEIGHT_NORMAL)
            label.SetFont(font)
            ...
            if dialog.ShowModal() == wx.ID_YES:
                try:
                    ...
                    wx.MessageBox(
                        f"Error deleting file: {str(e)}", "Error", wx.OK | wx.ICON_ERROR
                    )
            ...
            if dialog.ShowModal() == wx.ID_OK:
                try:
                    ...
                    text_area.SetSelection(
                        line_pos, text_area.GetLineEndPosition(line_number)
                    )
                    # Update status bar
    ...
    def OnCustomize(self, event):
        ...
        try:
            ...
            wx.MessageBox(
                f"File '{file_name}' not found in the current directory.",
                "Error",
                wx.ICON_ERROR,
            )
            return
            ...
            wx.MessageBox(
                f"An error occurred while opening the file: {e}", "Error", wx.ICON_ERROR
            )
            return
        ...
        with open("theme.xcfg", "r") as file:
            ...
            # Default style
            text_area.StyleSetSpec(
                stc.STC_P_DEFAULT,
                f"fore:{light_text_color},italic,back:{dark_bg_color}",
            )
    ...
    def OnRunPylint(self, event):
        ...
        if current_tab:
            ...
            if self.output_window is None:
                self.output_window = wx.Dialog(
                    self, title="pylint Output", size=(600, 400)
                )
                ...
                self.output_text = wx.TextCtrl(
                    output_panel, style=wx.TE_MULTILINE | wx.TE_READONLY
                )
                output_vbox.Add(
                    self.output_text, proportion=1, flag=wx.EXPAND | wx.ALL, border=5
                )
    ...
    def ScanForViruses(self, file_name):
        ...
        try:
            ...
            log_text.AppendText(
                "Starting virus scan using VirusShare hash database...\n\n"
            )
            ...
            if not os.path.exists(temp_dir):
                os.mkdir(temp_dir)
            log_text.AppendText(
                f"Created temporary directory for hash databases: {temp_dir}\n"
            )
                ...
                try:
                    ...
                    def download_hash_file(file_num):
                        ...
                        local_path = os.path.join(
                            temp_dir, f"VirusShare_{file_num:03d}.md5"
                        )
                    ...
                    def check_hash_in_file(hash_file_path, target_hash):
                        try:
                            ...
                            log_text.AppendText(
                                f"Error reading hash file {os.path.basename(hash_file_path)}: {str(e)}\n"
                            )
                            return False
                    ...
                    for i in range(1, total_db_files + 1):
                        ...
                        if hash_file:
                            log_text.AppendText(
                                f"Checking file against {os.path.basename(hash_file)}...\n"
                            )
                    ...
                    try:
                        ...
                        log_text.AppendText(
                            f"Error removing temporary directory: {str(e)}\n"
                        )
                    ...
                    wx.CallAfter(
                        self.SetStatusText,
                        f"    Completed comprehensive virus scan for {file_name}",
                    )
            ...
            wx.MessageBox(
                f"Error setting up scan: {str(e)}", "Error", wx.OK | wx.ICON_ERROR
            )
    ...
    def RunWithAdminPrivileges(self, file_name):
        ...
        try:
            ...
            wx.MessageBox(
                f"Error launching with admin privileges: {str(e)}",
                "Error",
                wx.OK | wx.ICON_ERROR,
            )
    ...
    def RunInTerminal(self, file_name):
        ...
        arg_dialog = wx.TextEntryDialog(
            self, "Enter command-line arguments (optional):", "Run with Arguments"
        )
        args = ""
        ...
        try:
            ...
            wx.MessageBox(
                f"Error launching in terminal: {str(e)}", "Error", wx.OK | wx.ICON_ERROR
            )
            self.SetStatusText(f"    Error launching {file_name}")
    ...
    def OnFileOpen(self, event):
        ...
        if file_name:
            ...
            if (
                ...
                or file_name.endswith(".msi")
            ):
                # Add dialog to determine how to handle the executable
                dialog = wx.Dialog(
                    self, title="Executable File Options", size=(400, 250)
                )
                panel = wx.Panel(dialog)
                vbox = wx.BoxSizer(wx.VERTICAL)
                ...
                desc = wx.StaticText(
                    panel,
                    label=f"Selected executable: {file_name}\nHow would you like to proceed?",
                )
                ...
                run_cli_btn.Bind(
                    wx.EVT_BUTTON,
                    lambda evt, f=file_name: (
                        self.RunInTerminal(f),
                ...
                scan_virus_btn.Bind(
                    wx.EVT_BUTTON,
                    lambda evt, f=file_name: (
                        self.ScanForViruses(f),
                        dialog.EndModal(wx.ID_OK),
                    ),
                )
                cancel_btn.Bind(
                    wx.EVT_BUTTON, lambda evt: dialog.EndModal(wx.ID_CANCEL)
                )
            ...
            try:
                ...
                try:
                    ...
                    wx.MessageBox(
                        f"Error reading file: {str(e)}", "Error", wx.OK | wx.ICON_ERROR
                    )
                    return
            except Exception as e:
                wx.MessageBox(
            ...
            # Create minimap
            minimap = stc.StyledTextCtrl(
                editor_splitter, style=wx.TE_MULTILINE | wx.TE_READONLY
            )
            ...
            editor_splitter.SetSashGravity(
                0.85
            )  # Set main editor to take up most space
            ...
            for text_area in (text_area, minimap):
                ...
                if file_name.endswith(".py"):
                    ...
                    text_area.StyleSetSpec(
                        ...
                        f"fore:{cmt_color},italic,back:{dark_bg_color}",
                    )
                    text_area.StyleSetSpec(
                        stc.STC_P_STRING,
                        f"fore:{string_color},italic,back:{dark_bg_color}",
                    )
                    text_area.StyleSetSpec(
                        stc.STC_P_WORD,
                        f"fore:{keyword_color},bold,back:{dark_bg_color}",
                    )
                    text_area.SetKeyWords(
                        0,
                    ...
                    text_area.StyleSetSpec(
                        stc.STC_P_OPERATOR,
                        f"fore:{operator_color},bold,back:{dark_bg_color}",
                    )
                    text_area.StyleSetSpec(
                        stc.STC_P_NUMBER,
                        f"fore:{number_color},italic,back:{dark_bg_color}",
                    )
                    text_area.StyleSetSpec(
                        stc.STC_P_DECORATOR,
                        f"fore:{keyword_color},italic,back:{dark_bg_color}",
                    )

                    # Strings
                    text_area.StyleSetSpec(
                        stc.STC_P_STRING, f"fore:{string_color},back:{dark_bg_color}"
                    )  # Regular strings
                    text_area.StyleSetSpec(
                        stc.STC_P_CHARACTER, f"fore:#FF79C6,bold,back:{dark_bg_color}"
                    )  # Character strings (we'll use this for prefixed strings)
                    text_area.StyleSetSpec(
                        stc.STC_P_TRIPLE, f"fore:{string_color},back:{dark_bg_color}"
                    )  # Triple quotes
                    text_area.StyleSetSpec(
                        stc.STC_P_TRIPLEDOUBLE,
                        f"fore:{string_color},back:{dark_bg_color}",
                    )  # Triple double quotes
                    text_area.StyleSetSpec(
                        stc.STC_P_DEFNAME, f"fore:#50FA7B,back:{dark_bg_color}"
                    )

                    # Keywords
                    text_area.StyleSetSpec(
                        stc.STC_P_WORD, f"fore:#569CD6,bold,back:{dark_bg_color}"
                    )
                    text_area.SetKeyWords(
                        0,
                        "def class return if else elif import from as not is try except finally for while in with pass lambda",
                    ...
                    text_area.StyleSetSpec(
                        stc.STC_P_IDENTIFIER,
                        f"fore:#7BCCE1,italic,back:{dark_bg_color}",
                    )

                    # Operators
                    text_area.StyleSetSpec(
                        stc.STC_P_OPERATOR, f"fore:#D4D4D4,bold,back:{dark_bg_color}"
                    )

                    # Numbers
                    text_area.StyleSetSpec(
                        stc.STC_P_NUMBER, f"fore:#FFDD54,italic,back:{dark_bg_color}"
                    )

                    # Decorators
                    text_area.StyleSetSpec(
                        stc.STC_P_DECORATOR, f"fore:#C586C0,italic,back:{dark_bg_color}"
                    )
                    ...
                    # Tags
                    text_area.StyleSetSpec(
                        stc.STC_H_TAG, f"fore:#569CD6,bold,back:{dark_bg_color}"
                    )

                    # Attributes
                    text_area.StyleSetSpec(
                        stc.STC_H_ATTRIBUTE, f"fore:#D69D85,italic,back:{dark_bg_color}"
                    )

                    # Attribute values
                    text_area.StyleSetSpec(
                        stc.STC_H_VALUE, f"fore:#BA9EFE,italic,back:{dark_bg_color}"
                    )
                    ...
                    # Entities
                    text_area.StyleSetSpec(
                        stc.STC_H_ENTITY, f"fore:#FFDD54,italic,back:{dark_bg_color}"
                    )

                    # Numbers
                    text_area.StyleSetSpec(
                        stc.STC_H_NUMBER, f"fore:#FFDD54,italic,back:{dark_bg_color}"
                    )

                    # Operators (like '=')
                    text_area.StyleSetSpec(
                        stc.STC_H_OTHER, f"fore:#D4D4D4,bold,back:{dark_bg_color}"
                    )
                    ...
                    text_area.StyleSetSpec(
                        stc.STC_JSON_STRING, f"fore:#BA9EFE,italic,back:{dark_bg_color}"
                    )

                    # Numbers (e.g., 123, 3.14)
                    text_area.StyleSetSpec(
                        stc.STC_JSON_NUMBER, f"fore:#FFDD54,back:{dark_bg_color}"
                    )

                    # Colons (e.g., in "key": "value")
                    text_area.StyleSetSpec(
                        stc.STC_JSON_OPERATOR, f"fore:#D4D4D4,bold,back:{dark_bg_color}"
                    )

                    # Keywords (e.g., true, false, null)
                    text_area.StyleSetSpec(
                        stc.STC_JSON_KEYWORD, f"fore:#68C147,bold,back:{dark_bg_color}"
                    )
                    ...
                    # Default text
                    text_area.StyleSetSpec(
                        stc.STC_CSS_DEFAULT, f"fore:#D4D4D4,back:{dark_bg_color}"
                    )
                    ...
                    text_area.StyleSetSpec(
                        stc.STC_CSS_TAG, f"fore:#569CD6,bold,back:{dark_bg_color}"
                    )

                    # Class and IDs (e.g., .className, #idName)
                    text_area.StyleSetSpec(
                        stc.STC_CSS_CLASS, f"fore:#7BCCE1,italic,back:{dark_bg_color}"
                    )
                    text_area.StyleSetSpec(
                        stc.STC_CSS_ID, f"fore:#FFAA33,italic,back:{dark_bg_color}"
                    )

                    # Attributes (e.g., color, margin, padding)
                    text_area.StyleSetSpec(
                        stc.STC_CSS_ATTRIBUTE, f"fore:#BA9EFE,bold,back:{dark_bg_color}"
                    )

                    # Pseudo-classes and Elements (e.g., :hover, ::before)
                    text_area.StyleSetSpec(
                        stc.STC_CSS_PSEUDOCLASS,
                        f"fore:#C586C0,italic,back:{dark_bg_color}",
                    )
                    text_area.StyleSetSpec(
                        stc.STC_CSS_PSEUDOELEMENT,
                        f"fore:#C586C0,italic,back:{dark_bg_color}",
                    )

                    # Property Values (e.g., red, 10px, 1em)
                    text_area.StyleSetSpec(
                        stc.STC_CSS_VALUE, f"fore:#FFDD54,back:{dark_bg_color}"
                    )

                    # Operators (e.g., :, ;, {, })
                    text_area.StyleSetSpec(
                        stc.STC_CSS_OPERATOR, f"fore:#D4D4D4,bold,back:{dark_bg_color}"
                    )

                    # Import Statement (e.g., @import)
                    text_area.StyleSetSpec(
                        stc.STC_CSS_DIRECTIVE, f"fore:#68C147,bold,back:{dark_bg_color}"
                    )
                    ...
                    # Default text
                    text_area.StyleSetSpec(
                        stc.STC_ESCRIPT_DEFAULT, f"fore:#D4D4D4,back:{dark_bg_color}"
                    )
                    ...
                    text_area.StyleSetSpec(
                        stc.STC_ESCRIPT_WORD, f"fore:#569CD6,bold,back:{dark_bg_color}"
                    )

                    # Strings (e.g., "text", 'text', `template literal`)
                    text_area.StyleSetSpec(
                        stc.STC_ESCRIPT_STRING,
                        f"fore:#BA9EFE,italic,back:{dark_bg_color}",
                    )

                    # Numbers (e.g., 123, 3.14, 0xFF)
                    text_area.StyleSetSpec(
                        stc.STC_ESCRIPT_NUMBER, f"fore:#FFDD54,back:{dark_bg_color}"
                    )

                    # Identifiers (e.g., variables, function names)
                    text_area.StyleSetSpec(
                        stc.STC_ESCRIPT_IDENTIFIER, f"fore:#D4D4D4,back:{dark_bg_color}"
                    )

                    # Operators (e.g., =, +, -, *, /, &&, ||)
                    text_area.StyleSetSpec(
                        stc.STC_ESCRIPT_OPERATOR,
                        f"fore:#D4D4D4,bold,back:{dark_bg_color}",
                    )

                    # Set JavaScript Keywords
                    text_area.SetKeyWords(
                        0,
                        "var let const function return if else for while do break continue switch case default try catch throw new this super class extends export import async await typeof instanceof delete",
                    )
                ...
                text_area.StyleSetSpec(
                    stc.STC_STYLE_LINENUMBER,
                    f"fore:{light_text_color},back:{line_number_bg}",
                )
                text_area.SetMarginType(1, stc.STC_MARGIN_NUMBER)
    ...
    def OnNewFile(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            if not file_input:
                wx.MessageBox(
                    "File name cannot be empty.", "Error", wx.OK | wx.ICON_ERROR
                )
                dialog.Destroy()
                return
    ...
    def OnRunCode(self, event):
        ...
        if current_tab:
            ...
            self.memory_thread = threading.Thread(
                target=self.track_memory_usage, daemon=True
            )
            ...
            if file_ext == ".py":
                ...
                # Compile first
                compile_process = subprocess.run(
                    ["g++", temp_file, "-o", "temp.exe"], capture_output=True, text=True
                )
                if compile_process.returncode == 0:
                    self.process = subprocess.Popen(
                        ["./temp.exe"],
                ...
                wx.MessageBox(
                    f"Unsupported file type: {file_ext}", "Error", wx.OK | wx.ICON_ERROR
                )
                return
            ...
            threading.Thread(
                target=self.HandleExecution, args=(overall_time,), daemon=True
            ).start()
    ...
    def HandleExecution(self, start_time):
        ...
        if self.output_window is None:
            ...
            self.return_text = wx.TextCtrl(
                return_panel, style=wx.TE_MULTILINE | wx.TE_READONLY
            )
            return_vbox.Add(
                self.return_text, proportion=1, flag=wx.EXPAND | wx.ALL, border=5
            )
        ...
        return_visualization = "\n".join(
            f"Return {i + 1}: {value}" for i, value in enumerate(self.return_values)
        )
            ...
            if file_name == "Untitled":
                ...
                if save_dialog.ShowModal() == wx.ID_OK:
                    ...
                    self.notebook.SetPageText(
                        self.notebook.GetSelection(), os.path.basename(file_name)
                    )
            else:
            ...
            try:
                ...
                if chr(key_code).isalpha() or key_code == ord("."):
                    ...
                    if length >= 0 or key_code == ord("."):
                        ...
                        if current_word:
                            all_completions = [
                                w for w in all_completions if w.startswith(current_word)
                            ]
