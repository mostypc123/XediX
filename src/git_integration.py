...
def commit():
    try:
        ...
        with wx.TextEntryDialog(
            None, "Enter Commit Message", "Git Commit"
        ) as msg_dialog:
            if msg_dialog.ShowModal() == wx.ID_OK:
                ...
                if not msg:
                    wx.MessageBox(
                        "Commit message cannot be empty", "Error", wx.OK | wx.ICON_ERROR
                    )
                    return
                ...
                try:
                    ...
                    wx.MessageBox(
                        "Commit successful!", "Success", wx.OK | wx.ICON_INFORMATION
                    )

                except subprocess.CalledProcessError as e:
                    wx.MessageBox(
                        f"Git commit failed: {e}", "Error", wx.OK | wx.ICON_ERROR
                    )
...
def add():
    try:
        ...
        subprocess.run(["git", "add", "."], check=True)
        wx.MessageBox(
            "Files added successfully!", "Success", wx.OK | wx.ICON_INFORMATION
        )
...
def version():
    try:
        ...
        wx.MessageBox(
            f"Git version: {version.stdout}", "Git Version", wx.OK | wx.ICON_INFORMATION
        )
