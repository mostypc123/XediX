...
class MergeResolver(wx.Dialog):
    def __init__(self, parent):
        ...
        current_sizer.Add(
            wx.StaticText(current_panel, label="Current Changes"), 0, wx.ALL, 5
        )
        ...
        incoming_sizer.Add(
            wx.StaticText(incoming_panel, label="Incoming Changes"), 0, wx.ALL, 5
        )
    ...
    def on_custom_resolve(self, event):
        custom_dialog = CustomResolveDialog(
            self, self.current_editor.GetText(), self.incoming_editor.GetText()
        )
    ...
    def _resolve_conflict(self, use_current):
        text = (
            ...
            if use_current
            else self.incoming_editor.GetText()
        )
...
class CustomResolveDialog(wx.Dialog):
    def __init__(self, parent, current_text, incoming_text):
        ...
        self.editor.SetText(
            f"# Combine the changes as needed:\n\n# Current changes:\n{current_text}\n\n# Incoming changes:\n{incoming_text}"
        )
